...
class UNIACCChatDemo {
    ...
    initializeElements() {
        ...
        this.testLogsDiv = document.getElementById('testLogs');
        this.testStatusDiv = document.getElementById('testStatus');
        this.stopTestsBtn = document.getElementById('stopTests');
        this.testHistoryDiv = document.getElementById('testHistory');
        // Metrics elements
        this.totalTestsSpan = document.getElementById('totalTests');
        this.successRateSpan = document.getElementById('successRate');
        this.avgResponseTimeSpan = document.getElementById('avgResponseTime');
        this.concurrentActiveSpan = document.getElementById('concurrentActive');
    ...
    switchTab(tabName, activeTab, otherTabs, activeContent, otherContents) {
        // Update tab styles
    ...
    async runConcurrentTest() {
        const concurrentUsersSelect = document.getElementById('concurrentUsers');
    ...
    async runConcurrentUserTest(userId, userNumber) {
        const startTime = performance.now();
        try {
    ...
    async sendTestMessage(userId, message) {
        const response = await fetch('/test-chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    logToTestPanel(message, type = 'info') {
        if (!this.testLogsDiv)
            return;
    ...
    updateTestStatus(message, isActive) {
        if (!this.testStatusDiv || !this.stopTestsBtn)
            return;
    ...
    addToTestHistory(testName, success, duration) {
        if (!this.testHistoryDiv)
            return;
        const timestamp = new Date().toLocaleTimeString();
    ...
    updateMetricsDisplay() {
        if (!this.totalTestsSpan || !this.successRateSpan || !this.avgResponseTimeSpan || !this.concurrentActiveSpan)
            return;
    ...
    async executeFlowTest(testCase) {
        const startTime = performance.now();
        ...
        try {
            ...
            for (let i = 0; i < testCase.messages.length; i++) {
                ...
                const keywordsInResponse = testCase.expectedKeywords.filter(keyword => response.toLowerCase().includes(keyword.toLowerCase()));
